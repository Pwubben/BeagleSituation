...
LBP & LBP::calcLBP( Mat d_img, double radius, bool borderCopy ) {
 ...
 //       double>();
	...
	//max(min(samples, 31), 1); // set bounds
	Mat result = Mat::zeros(d_img.rows - 2 * radius, d_img.cols - 2 * radius, CV_32FC1); //! result
																			  // allocate some memory for temporary on-line variance calculations
	Mat _mean = Mat::zeros(d_img.rows, d_img.cols, CV_32FC1);
	Mat _delta = Mat::zeros(d_img.rows, d_img.cols, CV_32FC1);
	Mat _m2 = Mat::zeros(d_img.rows, d_img.cols, CV_32FC1);
	

	for (int n = 0; n<samples; n++) {
		// sample points
		float x = static_cast<float>(radius) * cos(2.0*M_PI*n / static_cast<float>(samples));
		float y = static_cast<float>(radius) * -sin(2.0*M_PI*n / static_cast<float>(samples));
		// relative indices
		...
		int rows = d_img.rows-radius;
		int cols = d_img.cols - radius;
		
	#pragma omp parallel for 
		for (int i = radius; i < rows; i++) {
			for (int j = radius; j < cols; j++) {
				float t = w1*d_img.at<double>(i + fy, j + fx) + w2*d_img.at<double>(i + fy, j + cx) + w3*d_img.at<double>(i + cy, j + fx) + w4*d_img.at<double>(i + cy, j + cx);
	...
	// calculate result
   //#pragma omp parallel for 
	for (int i = radius; i < d_img.rows - radius; i++) {
		for (int j = radius; j < d_img.cols - radius; j++) {
			result.at<float>(i - radius, j - radius) = _m2.at<float>(i, j) / (1.0*(samples - 1));
		}
	}
	...
	//imshow("result", lbpImage);
	//cout << "ret (python)  = " << endl << format(result, Formatter::FMT_PYTHON) << endl << endl;
