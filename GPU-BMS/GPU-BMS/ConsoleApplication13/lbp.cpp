...
void lbp::VARLBP_(const Mat& src, Mat& dst, int radius, int neighbors) {
	...
	dst = Mat::zeros(src.rows*radius, src.cols*radius, CV_32FC1); //! result
	...
	cout << "ret (python)  = " << endl << format(_m2, Formatter::FMT_PYTHON) << endl << endl;
	// calculate result
	for(int i = radius; i < src.rows-radius; i++) {
		for(int j = radius; j < src.cols-radius; j++) {
...
template <typename _Tp>
void lbp::variance_lbp_(const Mat& src, Mat& _m2, int radius, int neighbors) {
	max(min(neighbors, 31), 1); // set bounds
	//_m2 = Mat::zeros(src.rows*radius, src.cols*radius, CV_32FC1); //! result
																  // allocate some memory for temporary on-line variance calculations
	Mat _mean = Mat::zeros(src.rows, src.cols, CV_32FC1);
	Mat _delta = Mat::zeros(src.rows, src.cols, CV_32FC1);
	_m2 = Mat::zeros(src.rows, src.cols, CV_32FC1);
	for (int n = 0; n<neighbors; n++)
	{
		// sample points
		float x = static_cast<float>(radius) * cos(2.0*M_PI*n / static_cast<float>(neighbors));
...
void lbp::variance_lbp(const Mat& src, Mat& _m2, int radius, int neighbors) {
	switch (src.type()) {
	case CV_8SC1: variance_lbp_<char>(src, _m2, radius, neighbors); break;
	case CV_8UC1: variance_lbp_<unsigned char>(src, _m2, radius, neighbors); break;
	case CV_16SC1: variance_lbp_<short>(src, _m2, radius, neighbors); break;
	case CV_16UC1: variance_lbp_<unsigned short>(src, _m2, radius, neighbors); break;
	case CV_32SC1: variance_lbp_<int>(src, _m2, radius, neighbors); break;
	case CV_32FC1: variance_lbp_<float>(src, _m2, radius, neighbors); break;
	case CV_64FC1: variance_lbp_<double>(src, _m2, radius, neighbors); break;
	}
}
...
Mat lbp::variance_lbp(const Mat& src, int radius, int neighbors) { Mat _m2; variance_lbp(src, _m2, radius, neighbors); return _m2; }
